  ...
  <section>
    ...
    <el-col :span="24" class="search">
      ...
      <el-form :inline="true">
        <el-form-item label="活动名称：">
          <el-input v-model="search_name1" placeholder="活动名称"></el-input>
        </el-form-item>
        <el-form-item label="活动ID：">
          <el-input v-model="search_id1" placeholder="活动ID"></el-input>
        </el-form-item>
    ...
    <el-col :span="24" class="table">
      <div class="table-title title">
        ...
        <span class="right" v-if="power.indexOf(',创建关联活动,') > -1" @click="foundactive"><i class="fa fa-plus fw"></i>
          创建关联活动</span>
      </div>
      <el-table ref="multipleTable" border :data="tabledata" highlight-current-row v-loading="listLoading"
        ...
        <el-table-column align="center" sortable prop="iclubEventId" label="车友会活动ID">
        </el-table-column>
        <el-table-column align="center" sortable prop="activityName" label="活动名称">
        </el-table-column>
        <el-table-column align="center" sortable prop="createTime" label="创建时间">
        </el-table-column>
        <el-table-column align="center" sortable prop="createName" label="创建人">
        </el-table-column>
        <el-table-column align="center" label="活动有效期">
          <template slot-scope="scope">
            <p>{{ scope.row.startTime }}</p>
            <p>{{ scope.row.endTime }}</p>
          </template>
        </el-table-column>
        <el-table-column align="center" label="启用" v-if="power.indexOf(',启用,') > -1">
          <template slot-scope="scope">
    ...
    <el-col :span="24" class="toolbar">
      <div class="tool-left" v-if="power.indexOf(',批量关闭活动,') > -1 || power.indexOf(',批量开启活动,') > -1">
        ...
        <el-dropdown trigger="click">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-if="power.indexOf(',批量关闭活动,') > -1" :disabled="sels.length == 0"
              @click.native="handleallClose">关闭活动</el-dropdown-item>
            <el-dropdown-item v-if="power.indexOf(',批量开启活动,') > -1" :disabled="sels.length == 0"
              @click.native="handleallOpen">开启活动</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
...
<script>
import { activeList, deleteActive, batchOpenActive } from "../../api/api";
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      let para = {
        activityStatus: this.search_status,
        activityName: this.search_name,
        iclubEventId: this.search_id,
        current: this.page,
        size: this.pagesize
      };
      this.listLoading = true;
      activeList(para).then(res => {
      ...
      this.$confirm("确认关闭这批活动吗?", "提示", {
        ...
        .then(() => {
          ...
          for (let i = 0; i < this.sels.length; i++) {
            para.ids.push(this.sels[i].activityId);
          }
          batchOpenActive(para).then(res => {
            if (res.header.code !== 10000000) {
              this.$message({
