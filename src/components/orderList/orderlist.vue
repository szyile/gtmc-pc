<template>
  <section>
      ...
      <div class="tu" style="position: relative; width: 100%; height: 82px;">
      <div class="ding">
    ...
    <!-- 数据统计 -->
    <!--
    <el-col :span="24" class="alldata">
      <el-form :inline="true" v-loading="listLoading1">
        <el-form-item>
    ...
    <el-col :span="24" class="search">
      ...
      <el-form :inline="true" style="display: block">
        ...
        <img src="../../../static/images/home/prompt.png"  @click="ShowStatus" />
        <el-form-item label="订单类型：">
          <el-select v-model="search_businessType1">
            <el-option label="预约保养" value="1"></el-option>
            <el-option label="便捷下单" value="2"></el-option>
          </el-select>
        </el-form-item>
        <div class="statusImg">
        <img src="../../../static/images/home/prompt.png"  @click="ShowStatus" />
    ...
    <el-col :span="24" class="table">
      ...
      <template>
        <div class="style" @click="allTime" :style="store_avatar==1?'background-color: #66b1ff;;color: white':'background-color: #f5f5f5'">
          <img src="../../../static/images/home/headurl.png" class="imgs">全部订单
        ...
        <div class="style" @click="normalTime" :style="store_avatar==2?'background-color: #66b1ff;;color: white':'background-color: #f5f5f5'">
          <img src="../../../static/images/home/headurl.png" class="imgs">派单正常
        ...
        <div class="style" @click="overTime" :style="store_avatar==3?'background-color: #66b1ff;;color: white':'background-color: #f5f5f5'">
          <img src="../../../static/images/home/headurl.png" class="imgs">派单超时
          <span class="two">{{this.search_overNum}}</span>件
        </div>
      </template>

      <el-table ref="multipleTable" border :data="orders" highlight-current-row v-loading="listLoading"
        ...
        <el-table-column align="center" label="订单类型">
          <template slot-scope="scope">
            {{ scope.row.businessType == '1' ?  '预约保养' : '' }}
            {{ scope.row.businessType == '2' ?  '便捷下单' : '' }}
          </template>
        </el-table-column>
        <el-table-column align="center" sortable prop="orderId" label="订单编号">
          <template slot-scope="scope">

              <span v-show="scope.row.assignTime>='30'">
                <img src="../../../static/images/home/headurl.png" />  {{ scope.row.orderId}}
              </span>
              <span v-show="scope.row.assignTime !=null &&scope.row.assignTime<'30'" >
                <img src="../../../static/images/user/add.png" />  {{ scope.row.orderId}}
              </span>
              <span v-show="scope.row.assignTime ==null" >
                {{ scope.row.orderId}}
              </span>
          </template>
        </el-table-column>

        <el-table-column align="center" sortable prop="assignTime" label="派单时长" v-if="">
          <template slot-scope="scope">
              <span v-show="scope.row.assignTime !=null && scope.row.applyStatus=='1'" style="color:#FB6260">
              {{ scope.row.assignTime}}
              </span>
            <span v-show="scope.row.assignTime !=null && scope.row.applyStatus=='0'">
              {{ scope.row.assignTime}}
              </span>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" fixed="right" label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.orderStatus=='10' || scope.row.orderStatus=='20'" class="el-button--table" size="small" icon="fa fa-pencil fa-fw"
              @click="handleEdit(scope.$index, scope.row)">修改订单
            </el-button>
            <el-button v-if="scope.row.orderStatus=='38' && scope.row.serviceType==1" class="el-button--table" size="small"
              @click="handleDown(scope.$index, scope.row)">
              取车下单
            </el-button>
            <el-button v-if="scope.row.orderStatus=='38' && scope.row.serviceType==2" class="el-button--table" size="small"
                       @click="handleDown(scope.$index, scope.row)">
              送车下单
            </el-button>
            <el-button v-if="scope.row.orderStatus=='10' || scope.row.orderStatus=='20' || scope.row.orderStatus=='38' || scope.row.orderStatus=='30'" class="el-button--table"
              size="small" @click="handleCancel(scope.$index, scope.row)">关闭订单
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class="tool-left" v-if="power.indexOf(',批量关闭订单,') > -1">
        ...
        <el-dropdown trigger="click">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-if="power.indexOf(',批量关闭订单,') > -1" :disabled="sels.length == 0"
              @click.native="handleallCan">关闭订单</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
    ...
    <!-- 关闭订单对话框 -->
    <el-dialog title="关闭订单" :before-close="close_cancel" :visible.sync="closeFormVisible" :close-on-click-modal="false"
      width="600px">
      <el-form class="el-close-order" ref="closeForm" :model="closeForm" :rules="closeFormRules" label-width="100px">
        ...
        <el-form-item prop="cancelReason" label="关闭说明：">
          <el-input type="textarea" placeholder="请输入关闭说明" maxlength="300" @input="close_num"
            v-model="closeForm.cancelReason">
          </el-input>
...
<script>
  import {batchCancel, orderCancel, orderCount, orderList} from "../../api/api";
